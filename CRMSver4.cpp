...
void addCustIntoFile()
{
  ...
  for (int i = 0; i < count; i++)
    cust << cust_idA[i] << "," << cust_nameA[i] << "," << C_passwordA[i] << endl;
  cust.close();
}
...
void loadCustomerIntoArray()
{
  ...
  while (getline(cust, word))
  {
    
    cust_idA[idx] = stoi(parseRecord(word, 1));
    cust_nameA[idx] = parseRecord(word, 2);
    C_passwordA[idx] = parseRecord(word, 3);
    idx++;
  }
  cust.close();
}
...
void loadCarIntoArray()
{
  ...
  while (getline(car, word))
  {
    
    car_nameA[idx] = parseRecord(word, 1);
    car_rentA[idx] = stoi(parseRecord(word, 2));
    idx++;
  }
  car.close();
}