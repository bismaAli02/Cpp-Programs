...
main()
{
 ...
 {
 cout << "Enter New Capacity for " <<
department[dpIndex - 1] << endl;
 cin >> capacity;
...
student_matric[st_index] << "\t" << student_fsc[st_index] << "\t" <<
student_ecat[st_index] << "\t" << aggr[st_index] << endl;
 }
...
{ //______Add user, only admin can add_________
   ...
   { 
     ...
     userCount = userCount + 1;
        ...
        cout<<"No more space to add user";
}
float aggrigate(float mMarks, float fMarks, float eMarks)
...
// }
 //string campare(string user_name, int password)
// {
//        for(int i = 0; i < MAX_RECORDS; i++)
//        {
//            if(user_nameA[i] == user_name && passwordsA[i] == password)
//                 return rolesA[i];
//        }
//        return "Wrong data "; 
